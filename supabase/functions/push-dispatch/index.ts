...
Deno.serve(async(req)=>{
 ...
 for(const job of jobs||[]){
  try{
   ...
   await supabase.from('push_outbox').update({sent_at:new Date().toISOString(),last_error:null}).eq('id',job.id);sent++;
  }catch(e:any){const attempts=Number(job.attempt_count||0)+1;const terminal=attempts>=MAX_ATTEMPTS;await supabase.from('push_outbox').update({attempt_count:attempts,last_attempt_at:new Date().toISOString(),last_error:String(e?.message||e).slice(0,500),sent_at:terminal?new Date().toISOString():null}).eq('id',job.id);failed++}
 }
