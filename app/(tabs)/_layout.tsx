...
export default function TabsLayout(){
 ...
 useEffect(()=>{if(!session)return;refreshUnread();const channel=supabase.channel('tab-unread-'+session.user.id).on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},()=>refreshUnread()).on('postgres_changes',{event:'*',schema:'public',table:'notifications',filter:`user_id=eq.${session.user.id}`},()=>refreshUnread()).subscribe();const timer=setInterval(refreshUnread,5000);return()=>{clearInterval(timer);supabase.removeChannel(channel)}},[session,refreshUnread]);
